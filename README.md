...
- **按比例缩放**：10%～100% 可选（默认 60%），最长边占视口对应尺寸的比例
  （100% = 最长边正好铺满视口，**不是** 1:1 像素；状态栏「缩放：xx%」显示的才是实际像素缩放）；
- **bg / bg 拷贝 可各用一份独立比例**（默认关闭）：开启后只有这两类图层用独立值，
  其余图层仍用全局比例；「bg 拷贝 / bg copy」没有这一层就不套用、也不兜底，
  bg 没有则按既有规则退回「最底部有可用像素内容的图层」；开关关闭时两个下拉置灰；
...
- 桌面端设置对话框分为五组：**显示**（显示比例 / bg 与 bg 拷贝的独立比例 / 问题红框范围 / 蓝色虚线框；界面缩放仅 Android 显示）、
